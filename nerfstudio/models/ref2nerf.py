...
from nerfstudio.fields.ref2nerf_field import Ref2NerfField

@dataclass
class Ref2NeRFModelConfig(ModelConfig):
    """Nerfacto Model Config"""

    _target: Type = field(default_factory=lambda: Ref2NeRFModel)
...
class Ref2NeRFModel(Model):
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        outputs = {
            "rgb": rgb,
            "rgb_vi": field_outputs[FieldHeadNames.VI_RGB],
            "rgb_vd": field_outputs[FieldHeadNames.VD_RGB],
            "offset": field_outputs[FieldHeadNames.SH],
    ...
    def get_image_metrics_and_images(
        ...
        vi_rgb = outputs["rgb_vi"]
        vd_rgb = outputs["rgb_vd"]

        combined_rgb = torch.cat([gt_rgb, predicted_rgb], dim=1)
        combined_vi_rgb = torch.cat([vi_rgb], dim=1)
        combined_vd_rgb= torch.cat([vd_rgb], dim=1)
